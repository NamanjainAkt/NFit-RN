...
Starts the continuous foreground service (`StepTrackerService`) on Android.

### `stopBackgroundService()`
Stops the foreground step tracking service.

### `isBackgroundServiceRunning()`
Checks if `StepTrackerService` is currently running.

### `getAccumulatedSteps()`
Reads accumulated steps from background tracking via `NfitBackgroundSteps.getAccumulatedSteps()`. Returns the total steps recorded while the app was closed. Android-only; returns 0 on other platforms.
